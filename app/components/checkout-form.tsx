...
import { WorldpayCardFields, type WorldpayCardHandle } from "./worldpay-card-fields";

type PaymentConfig = { stripe: boolean; worldpay: boolean; worldpayCheckoutId: string; worldpayEnvironment: "try" | "live"; deliveryFeePence: number };
...
export function CheckoutForm() {
  ...
  const worldpayRef = useRef<WorldpayCardHandle>(null);
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const worldpaySessions = provider === "worldpay" ? await worldpayRef.current?.generateSessions() : undefined;
      ...
      const result = await response.json() as { error?: string; redirectUrl?: string; actionRequired?: boolean; actionUrl?: string };
      if (!response.ok && !result.actionRequired) throw new Error(result.error || "Payment could not be started.");
      if (result.redirectUrl) window.location.assign(result.redirectUrl);
      else if (result.actionRequired) throw new Error("Worldpay requires additional card verification. Please use Stripe while the 3DS challenge is completed.");
  ...
  return (
    <main className="checkoutPage">
      <header className="checkoutHero"><p>Secure checkout · Motherwell</p><h1><span>Complete your</span><span>order.</span></h1><div><span>{lines.reduce((sum, line) => sum + line.quantity, 0)} items</span><i /><span>{formatPrice(totalPence)}</span></div></header>
      <form className="checkoutLayout" onSubmit={handleSubmit}>
        <div className="checkoutDetails">
          ...
          <section className="checkoutSection"><div className="checkoutSectionHeading"><span>03</span><div><p>Payment</p><h2>Choose your passage.</h2></div></div>{config && !config.stripe && !config.worldpay && <div className="paymentNotice">Payments are in setup mode. Add server credentials from <code>.env.example</code> to enable checkout.</div>}<div className="choiceCards paymentChoices"><label className={provider === "stripe" ? "isSelected" : ""}><input type="radio" name="provider" value="stripe" checked={provider === "stripe"} onChange={() => setProvider("stripe")} /><strong>Stripe</strong><span>{config?.stripe ? "Secure hosted checkout" : "Not configured"}</span></label><label className={provider === "worldpay" ? "isSelected" : ""}><input type="radio" name="provider" value="worldpay" checked={provider === "worldpay"} onChange={() => setProvider("worldpay")} /><strong>Worldpay</strong><span>{config?.worldpay ? "Secure embedded card fields" : "Not configured"}</span></label></div>{provider === "stripe" ? <p className="providerExplanation">You will continue to Stripe&apos;s secure checkout to complete payment.</p> : config?.worldpay && <WorldpayCardFields ref={worldpayRef} checkoutId={config.worldpayCheckoutId} environment={config.worldpayEnvironment} />}</section>
        </div>
