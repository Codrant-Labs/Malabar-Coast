...
export default async function ReservationsAdminPage({ searchParams }: { searchParams: Promise<{ update?: string }> }) {
  ...
  return <AdminFrame active="/admin/reservations" session={session}>
    ...
    {query.update && <p className={`adminAlert ${["success","deleted"].includes(query.update) ? "isSuccess" : "isError"}`}>{query.update === "deleted" ? "Reservation deleted from the active register." : query.update === "success" ? "Reservation updated." : "That update was rejected."}</p>}
